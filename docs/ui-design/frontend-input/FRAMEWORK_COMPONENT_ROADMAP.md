...
3. 图标映射收敛（Icon Mapping Reconciliation）：主导航、书架、发现、RSS、设置二级页、书源链路和共享状态组件已接入 `ReaderIconToken`；后续优先迁移阅读控制层直接 Material Icons，并同步素材库语义。
4. 状态矩阵落地（State Matrix Implementation）：主标签页已建立第一批 Compose preview/state matrix；后续扩展到书架链路、设置二级页、书源链路和阅读控制层。
