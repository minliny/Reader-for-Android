...
2. 正式 MainTabShell 落地（MainTabShell Implementation）：主导航代码已收敛到 `书架 / 发现 / RSS / 设置`，运行时底栏已切到 `ReaderMainTabShell`，书架根路由已脱离 `StitchAppShell`，RSS 根路由已接入 `RssHomeScreen`，设置根页已接入 `SettingsRootScreen` + `SettingsHomeState`；下一步补齐状态矩阵细节和图标 token 迁移。
