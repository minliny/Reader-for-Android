...
当前缺口：
- 横向页面目前只有 `换源` 一例，已沉淀 landscape frame 规则，但还需要后续更多横向设计图验证。
...
当前 `SettingsPageKit` 已覆盖 7 个设置二级页，是 `SettingsShell` 的第一版实现；`MainTabPageKit` 已覆盖 4 个主标签页，是 `MainTabShell` 的第一版实现；之后再抽 `LibraryShell` 和 `ReaderShell`。
...
- `StartReadingButton`：阅读入口开始按钮，必须表达从章节开头或指定章节重新打开。
- `ContinueReadingButton`：阅读入口继续按钮，必须保留当前章节和阅读进度。
- `SwitchSourceButton`：换源候选行的切换按钮，必须显示具体结果文案 `切换来源`，当前源和失效源不可提交。
- `SelectRow`：设置二级页选择项，必须展示当前值和进入底表的箭头，不在行内展开完整选项。
...
- `ThemeSwatch`：阅读背景主题色块，必须同时表达背景和文字对比。
...
- `ReplacementRuleRow`：内容替换规则行，包含规则名、命中说明、启用开关和进入编辑的语义。
- `RepairEntry`：阅读入口失败修复行，必须保留来源上下文并提供 `重试` 与 `更换来源`。
...
当前缺口：
- `SourceListItem`、`RSSSourceItem`、`NoteCard` 尚未完成。
- 行内 badge、开关、二级说明的规则还未完整。
...
- `OptionSheet`：设置选择项底表，必须有标题、当前项、取消和关闭入口。
...
当前缺口：
- `SortFilterSheet`、`AppearancePanel`、`TTSPanel`、`SourceSwitchPanel` 只是可由现有组件派生，尚未单独成规格。
...
- `OpenLoadingState`：阅读入口打开章节时的加载状态，必须显示 `正在打开` 并禁止重复提交。
