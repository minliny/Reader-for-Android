...
| `app/src/main/kotlin/com/reader/android/ui/settings/MineScreen.kt` | 新增正式 `SettingsRootScreen`、`SettingsHomeState`、`SettingsHomeMapper` 和 `SettingsHomeDisplayState`，内容已覆盖本地概览、常用管理、全部设置、概览加载、无备份和权限缺失状态；`MineScreen` 仅保留为 legacy wrapper | 已收敛（Done） | 后续继续补真实业务数据接入。 |
| `app/src/main/kotlin/com/reader/android/ui/discover/RssScreens.kt` | 新增 `RssHomeScreen` + `RssHomeDesignUiState` 作为 RSS 主 tab 内容，`RssListScreen` 保留为二级列表 | 已收敛（Done） | 后续接入真实订阅流数据。 |
...
4. 建设置首页（Settings Root）：已完成第一轮，主 tab 使用 `SettingsRootScreen` + `SettingsHomeMapper`，内容覆盖本地概览、常用管理、全部设置、概览加载、无备份和权限缺失状态；`MineScreen` 仅兼容 legacy route。后续继续补真实业务数据接入。
5. 建 RSS 根页（RSS Root）：已完成第一轮，`ReaderRoutes.RSS` 渲染 `RssHomeScreen`，包含订阅概览、筛选和最新订阅列表；Compose 输入矩阵已覆盖 default、loading、empty、unreadEmpty、error，后续接入真实订阅流数据。
