window.ReaderAssetLibraryFixture = {
  ...
  screenGroups: [
    ...
    },
    {
      ...
      items: [
        { name: "书架空状态（Library Empty）", path: "../../03-书架链路/书架空状态/UI设计图.png", shell: "LibraryShell" },
    ...
    },
    {
      ...
      items: [
        { name: "App 通用设置（General Settings）", path: "../../05-设置链路/App通用设置/UI设计图.png", shell: "SettingsShell" },
        { name: "书架与搜索设置（Bookshelf and Search Settings）", path: "../../05-设置链路/书架与搜索设置/UI设计图.png", shell: "SettingsShell" },
        { name: "隐私与权限（Privacy and Permissions）", path: "../../05-设置链路/隐私与权限/UI设计图.png", shell: "SettingsShell" },
        { name: "缓存管理（Cache Management）", path: "../../05-设置链路/缓存管理/UI设计图.png", shell: "SettingsShell" },
  ...
  iconGroups: [
    ...
    { title: "状态与动作图标（Status and Action Icons）", items: ["clock", "check", "warning", "info", "refresh", "sync", "sort", "people", "link", "message", "mail", "phone", "monitor", "gesture", "assist", "motion", "top", "list", "edit", "log", "add", "text", "typo"] }
  ],
  supplementedIcons: ["back", "more", "chevron", "close", "directory", "tts", "auto-page", "replace", "pause", "book-open", "source-stack", "progress", "volume", "gear"],
  usageRules: [
    "新增页面前必须先查询素材库。",
    "已有图标 token 不得新增同义名称。",
    "缺少图标时先补 icons.js，再进入页面 renderer。",
