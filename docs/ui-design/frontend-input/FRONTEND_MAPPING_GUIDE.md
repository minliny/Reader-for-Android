...
7. 验证覆盖（Validation Coverage）：主标签页、书源管理链路、设置二级页、阅读控制层、书架搜索/详情链路和换源 FlowShell 已建立第一批 Compose preview/state matrix；后续每个页面至少保留默认、加载、空、错误或模块展开态的 Compose preview / UI test / prototype gallery 状态。
