...
| `reader_overlay_default` | 沉浸页点击中间区域 | 点击正文/返回键/自动隐藏 | 是 | 是 | 是 | 是 | 是 | 是 | `04-reader-controls.png` | `CANDIDATE_OK` |
...
| `reader_overlay_directory` | 底栏 `目录` | 再次切换模块/点击章节/关闭控制层 | 是 | 是 | 是 | 是 | 可弱化或替换，需确认 | 是 | `16-5-reader-toc-quick-panel-height-aligned.png` | `CANDIDATE_OK` |
| `reader_overlay_tts` | 底栏 `朗读` | 开始朗读/切换模块/关闭控制层 | 是 | 是 | 是 | 是 | 可弱化或替换，需确认 | 是 | `18-3-read-aloud-quick-panel-height-aligned.png` | `CANDIDATE_OK` |
...
| `reader_overlay_appearance` | 底栏 `界面` | 切换模块/关闭控制层 | 是 | 是 | 是 | 是 | 可弱化或替换，需确认 | 是 | `21-1-reader-appearance-quick-panel.png` | `NEEDS_DECISION` |
| `reader_overlay_settings` | 底栏 `设置` | 切换模块/关闭控制层 | 是 | 是 | 是 | 是 | 可弱化或替换，需确认 | 是 | `22-2-reader-settings-quick-panel-height-aligned.png` | `CANDIDATE_OK` |
...
`16-5 / 18-3 / 19-1 / 22-2 / 25-4` 已完成按 `04-reader-controls.png` 的相对高度对齐。历史图 `16-4 / 18-2 / 19 / 22-1 / 25-1` 仅作为对照，不再作为当前采用版。
